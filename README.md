...
| v0.6 | Rich text composition (markdown runs, inline highlighting, embedded elements). | in progress — paragraph wrapping + text alignment landed earlier; v0.6.1 step A landed the inline-runs API surface (`text_runs([...])` + `hard_break()` + `Kind::Inlines` / `Kind::HardBreak` + new El text-mark fields and modifiers `.italic()` / `.underline()` / `.strikethrough()` / `.code()` / `.link(url)`). Step B (attributed cosmic-text shaping + per-glyph paint) is the next slice — `Kind::Inlines` currently falls through to column-flow as a placeholder. v0.6.2 (semantic / syntax highlighting) and v0.6.3 (inline embeds) queued behind it. Markdown integration lives outside core (`aetna-markdown`-style crate, deferred). |
