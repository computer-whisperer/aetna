...
- v0.9 — anchored popovers + `context_menu` / `dropdown` helpers. Kit growth: `LayoutCtx::rect_of_key(&str) → Option<Rect>` — a callback handed to every `layout_override` so custom layouts can position children relative to keyed elements outside their own subtree (the popover-anchors-trigger pattern, but the primitive is general — connecting lines, sticky headers, breadcrumb tails all use it). Backed by a `key → computed_id` index that the layout pass refreshes once per frame. New widgets in `widgets/popover.rs`: `Anchor` (`Key { key, side }` / `Point { x, y, side }`) + `Side` (`Below`/`Above`/`Right`/`Left`/`AtPoint`); `anchor_rect()` — pure positioning helper that flips to the opposite side on viewport overflow and clamps the secondary axis; `popover(key, anchor, panel)` — overlay + dismiss scrim + anchored panel layer (emits `{key}:dismiss` on click-outside); `popover_panel(items)` — bare card surface (no scrim) for tooltips; `menu_item(label)`; `dropdown(key, trigger_key, items)` and `context_menu(key, point, items)` as compositions. Apps own open/closed state and compose at the root via `stack([content, if open { popover(...) } else { ... }])` — explicit, no portal hoist. Demo: `bin/popover.rs` covers top dropdown, bottom dropdown (auto-flip-up), context menu, and a non-scrim tooltip composed against `anchor_rect` directly. Deferred to v0.9.x: auto-focus on open (a future `UiState::request_focus_key`), keyboard arrow-nav inside menu_item lists, hover-driven tooltip open.
