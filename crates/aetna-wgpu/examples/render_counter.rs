...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("aetna_wgpu::example::counter::device"),
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("aetna_wgpu::example::counter::target"),
        size: extent,
        mip_level_count: 1,
        sample_count: 1,
    ...
    let readback_buf = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("aetna_wgpu::example::counter::readback"),
        size: readback_size,
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("aetna_wgpu::example::counter::encoder"),
    });
    {
        let bg = bg_color();
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("aetna_wgpu::example::counter::pass"),
