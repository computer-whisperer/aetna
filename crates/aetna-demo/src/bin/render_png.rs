...
//!
//! Usage: `cargo run -p aetna-demo --bin render_png`
//! Writes: `crates/aetna-demo/out/settings.wgpu.png`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(
        &wgpu::DeviceDescriptor {
            label: Some("aetna_demo::headless::device"),
    ...
    let readback_buf = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("aetna_demo::headless::readback"),
        size: readback_size,
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("aetna_demo::headless::encoder"),
    });
    {
        let bg = bg_color();
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("aetna_demo::headless::pass"),
