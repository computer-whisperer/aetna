...
//! source location lives in [`El::source`] and flows through to the
//! tree dump and lint artifacts the agent loop consumes.
//!
//! # State
//!
//! Each `El` carries an [`InteractionState`] (default, hover, press,
//! focus, disabled, loading). The renderer applies state-specific
//! visual deltas. State is set with [`El::with_state`].
...
/// lint, and future render-time theme substitution.
...
/// transformation when emitting render commands.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Default)]
