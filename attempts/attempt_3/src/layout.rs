...
//! refactors and are usable as patch targets.
//!
//! Text intrinsic measurement is approximate (`chars × font_size × 0.56`).
//! Good enough for SVG fixtures; will be replaced when wgpu/vulkano
//! lands with real shaping.
...
fn layout_axis(node: &mut El, vertical: bool) {
    ...
    let mut cursor = match node.justify {
        Justify::Start => 0.0,
        Justify::Center => ((main_extent - used - free_after_used) * 0.5).max(0.0),
        Justify::End => (main_extent - used - free_after_used).max(0.0),
