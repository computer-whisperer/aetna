...
//! winit window.
//!
//! Usage: `cargo run -p attempt_4_demo --bin render_counter`
//! Writes: `attempts/attempt_4/out/counter.wgpu.png`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(
        &wgpu::DeviceDescriptor {
            label: Some("attempt_4_demo::counter::device"),
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("attempt_4_demo::counter::target"),
    ...
    let readback_buf = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("attempt_4_demo::counter::readback"),
        size: readback_size,
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("attempt_4_demo::counter::encoder"),
    });
    {
        let bg = bg_color();
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("attempt_4_demo::counter::pass"),
