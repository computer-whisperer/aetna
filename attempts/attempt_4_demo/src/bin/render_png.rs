...
//!
//! Usage: `cargo run -p attempt_4_demo --bin render_png`
//! Writes: `attempts/attempt_4/out/settings.wgpu.png`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(
        &wgpu::DeviceDescriptor {
            label: Some("attempt_4_demo::headless::device"),
    ...
    let readback_buf = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("attempt_4_demo::headless::readback"),
        size: readback_size,
    ...
    });
    {
        ...
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("attempt_4_demo::headless::pass"),
