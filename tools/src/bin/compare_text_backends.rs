...
//! MTSDF text path.
//!
//! Usage: `cargo run -p aetna-tools --bin compare_text_backends`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let wgpu_out = workspace.join("crates/aetna-wgpu/out");
...
fn report_markdown(rows: &[(Scale, DiffStats)]) -> String {
    let mut out = String::from(
        "# Text Backend Parity\n\n\
         Generated by `cargo run -p aetna-tools --bin compare_text_backends`.\n\n\
         Each sheet is laid out as: wgpu render, Vulkano render, amplified RGB diff.\n\
         The fixture comes from `aetna_fixtures::text_quality::fixture()` so both \
